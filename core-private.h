...
#define PRINT_ERR(...) \
	if (ctx->log_cb)   \
		ctx->log_cb(NULL, LIBKLVANC_LOGLEVEL_ERR, __VA_ARGS__);
#define PRINT_DEBUG(...) \
	if (ctx->log_cb)     \
		ctx->log_cb(NULL, LIBKLVANC_LOGLEVEL_DEBUG, __VA_ARGS__);
#define PRINT_DEBUG_MEMBER_INT(m) \
	if (ctx->log_cb)              \
		ctx->log_cb(NULL, LIBKLVANC_LOGLEVEL_DEBUG, " %s = 0x%x\n", #m, m);
#define PRINT_DEBUG_MEMBER_INTI(m, n) \
	if (ctx->log_cb)                  \
		ctx->log_cb(NULL, LIBKLVANC_LOGLEVEL_DEBUG, "%*c%s = 0x%x\n", n, ' ', #m, m);
#define PRINT_DEBUG_MEMBER_INT64(m) \
	if (ctx->log_cb)                \
		ctx->log_cb(NULL, LIBKLVANC_LOGLEVEL_DEBUG, " %s = 0x%" PRIx64 "\n", #m, m);
