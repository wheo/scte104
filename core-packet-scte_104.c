...
static const char *som_operationName(unsigned short opID)
{
	...
	case 0x0003:
		return "alive_request_data";
	case 0x0004:
		return "alive_response_data";
	case 0x0005:
	case 0x0006:
		return "User Defined";
	case 0x0007:
		return "inject_response_data";
	case 0x0008:
		return "inject_complete_response_data";
	case 0x0009:
		return "config_request_data";
	case 0x000a:
		return "config_response_data";
	case 0x000b:
		return "provisioning_request_data";
	case 0x000c:
		return "provisioning_response_data";
	case 0x000f:
		return "fault_request_data";
	case 0x0010:
		return "fault_response_data";
	case 0x0011:
		return "AS_alive_request_data";
	case 0x0012:
		return "AS_alive_response_data";
	default:
		return "Reserved";
	}
}
											   ...
											   struct klvanc_avail_descriptor_request_data *d)
{
	...
	{
		d->provider_avail_id[i] = *(p + 0) << 24 | *(p + 1) << 16 | *(p + 2) << 8 | *(p + 3);
		p += 4;
	}
															 ...
															 unsigned int descriptor_size)
{
	...
	d->proprietary_id = *(p + 0) << 24 | *(p + 1) << 16 | *(p + 2) << 8 | *(p + 3);
	p += 4;
